...
if (isset($_GET['table']) && $_GET['table'] == 'category') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['category_type'] = ($row['category_type']) ? '<label class="label label-primary">Paid</label>' : '<label class="label label-warning ">Free</label>';
...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    $sql = "SELECT users.*,IFNULL((SELECT IFNULL(SUM(points),0) FROM tbl_tracker WHERE coin_status='0' AND tbl_tracker.uid=users.firebase_id) + (SELECT IFNULL(SUM(points),0) FROM tbl_tracker WHERE coin_status='1' AND tbl_tracker.uid=users.firebase_id),0) as TotalTrackerCoins FROM `users` " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate .= "<a class='btn btn-xs btn-success' href='activity-tracker.php?uid=".$row['firebase_id']."' target='_blank' title='Track Activities'><i class='far fa-chart-bar'></i></a>";
	    $operate .= "<a class='btn btn-xs btn-danger' href='payment-requests.php?uid=".$row['firebase_id']."' target='_blank' title='Payment Requests'><i class='fas fa-rupee-sign'></i></a>";
        $operate .= "<a class='btn btn-xs btn-warning admin-coin' data-id='" . $row['id'] . "' data-toggle='modal' data-target='#coinsmodal' title='Coins'><i class='fas fa-coins'></i></a>";
        ...
        $tempRow['coins'] = $row['TotalTrackerCoins'];
...
if (isset($_GET['table']) && $_GET['table'] == 'question_reports') {
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
	        $where = " where (qr.id like '%" . $search . "%' OR message like '%" . $search . "%' OR u.name like '%" . $search . "%'  OR q.question like '%" . $search . "%')";
	    }
    ...
    $sql = "SELECT qr.*, u.name, q.category, q.subcategory, q.language_id, q.image, q.question, q.question_type, q.optiona, q.optionb, q.optionc, q.optiond, q.optione, q.answer, q.note FROM question_reports qr " . $join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
...
if (isset($_GET['table']) && $_GET['table'] == 'contest_questions') {
    ...
    if (isset($_GET['search'])) {
        ...
        if (!empty($_GET['contest_filter'])) {
            $contest_filter = $_GET['contest_filter'];
            $where .= " AND (q.id like '%" . $search . "%' OR q.question like '%" . $search . "%' OR q.optiona like '%" . $search . "%' OR q.optionb like '%" . $search . "%' OR q.optionc like '%" . $search . "%' OR q.optiond like '%" . $search . "%' OR `answer` like '%" . $search . "%' ) AND `contest_id`=" . $contest_filter;
        } else {
            $where = " WHERE (q.id like '%" . $search . "%' OR q.question like '%" . $search . "%' OR q.optiona like '%" . $search . "%' OR q.optionb like '%" . $search . "%' OR q.optionc like '%" . $search . "%' OR q.optiond like '%" . $search . "%' OR `answer` like '%" . $search . "%' ) ";
        }
    }
...
if (isset($_GET['table']) && $_GET['table'] == 'tracker') {
    ...
    if (isset($_GET['uid'])) {
        $uid = $_GET['uid'];
        if ($_GET['uid']!= '') {
            $where = "where (users.`firebase_id` = '".$uid."')";
        }
    }
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
         ...
         $where = " where (tbl_tracker.`id` like '%".$search."%' OR tbl_tracker.`uid` like '%".$search."%' OR users.`name` like '%".$search."%' OR tbl_tracker.`points` like '%".$search."%' OR tbl_tracker.`type` like '%".$search."%' OR tbl_tracker.`date` like '%".$search."%' )";
       
    }
    
    $left_join = " LEFT JOIN users  on users.firebase_id=tbl_tracker.uid";
    $sql = "SELECT COUNT(tbl_tracker.id) as total FROM `tbl_tracker` ".$left_join." ".$where;
    $db->sql($sql);
    ...
    $sql = "SELECT tbl_tracker.*,users.name FROM `tbl_tracker` ".$left_join." ".$where." ORDER BY ".$sort." ".$order." LIMIT ".$offset.", ".$limit;
...
if (isset($_GET['table']) && $_GET['table'] == 'payment_requests') {
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        if ($_GET['username']!= '') {
            $where .= " AND (pr.`id` like '%".$search."%' OR pr.`uid` like '%".$search."%' OR pr.`payment_address` like '%".$search."%' OR pr.`request_type` like '%".$search."%' OR pr.`request_amount` like '%".$search."%' OR pr.`points_used` like '%".$search."%' OR pr.`date` like '%".$search."%' )";
        } else {
            $where = " where (pr.`id` like '%".$search."%' OR pr.`uid` like '%".$search."%' OR pr.`uid` like '%".$search."%' OR pr.`request_type` like '%".$search."%' OR pr.`request_amount` like '%".$search."%' OR pr.`points_used` like '%".$search."%' OR pr.`date` like '%".$search."%' )";
        }
    }
    
    $left_join = "LEFT JOIN users u on u.firebase_id=pr.uid";
    $sql = "SELECT COUNT(`id`) as total FROM `payment_requests` pr ".$where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT pr.*,u.name as name FROM `payment_requests` pr ".$left_join."".$where." ORDER BY ".$sort." ".$order." LIMIT ".$offset.", ".$limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $tempRow['status'] = ($row['status']==1)?"<label class='label label-success'>Completed</label>":(($row['status']==0)?"<label class='label label-warning'>Pending</label>":"<label class='label label-danger'>Wrong details</label>");
        $tempRow['date'] = $row['date'];
        $tempRow['operate'] = $operate;
        $rows[] = $tempRow;
    }
