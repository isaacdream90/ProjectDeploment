...
// ------------------------------------------------- user purchased key -------------------------------
if (isset($_POST['user_purchase_category'])) {
    if (!verify_token()) {
        return false;
    }
    if ($access_key != $_POST['access_key']) {
    ...
    $sql = "INSERT INTO `user_purchased_category` (`user_id`,`category_id`) VALUES ('" . $user_id . "','" . $category_id . "')";
    $db->sql($sql);

    $response['error'] = "false";
    $response['data'] = "Purchased Successfully";
    print_r(json_encode($response));
    ...
    if (isset($_POST['language_id']) && !empty($_POST['language_id']) && isset($_POST['user_id']) && !empty($_POST['user_id'])) {
        ...
        if (!empty($result)) {
            for ($i = 0; $i < count($result); $i++) {
                ...
                $sql2 = "SELECT * FROM `tbl_last_play_status` WHERE user_id='$user_id'  AND  category_id='".$result[$i]['id']."' AND language_id='$language_id'";
                $db->sql($sql2);
                $result2= $db->getResult();
                $result[$i]['last_limit'] = (!empty($result2)) ? $result2[0]['last_limit'] : '0' ;

                  //-----------------is_purchased
                  $sql3 = "SELECT * FROM `user_purchased_category` WHERE user_id='$user_id'  AND  category_id='" . $result[$i]['id'] . "'";
                  $db->sql($sql3);
                  $result3 = $db->getResult();
  
                  $result[$i]['is_purchased'] = (!empty($result3)) ? '1' : '0';
            }
            $response['error'] = "false";
...
if (isset($_POST['access_key']) && isset($_POST['get_categories'])) {
    ...
    if (isset($_POST['id']) && isset($_POST['user_id']) && !empty($_POST['user_id'])) {
        ...
        $sql = "SELECT *,(select count(id) from question where question.category=c.id ) as no_of_que,
    (SELECT @no_of_subcategories := count(`id`) from subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of FROM `category` c WHERE c.id = $id ORDER By CAST(c.row_order as unsigned) ASC";
        $db->sql($sql);
        $result = $db->getResult();
        print_r($result);
        ...
        if (!empty($result)) {
            ...
            $sql2 = "SELECT * FROM `tbl_last_play_status` WHERE user_id='$user_id'  AND  category_id='$id' AND language_id='0'";
            $db->sql($sql2);
            $result2 = $db->getResult();
            $result[0]['last_limit'] = (!empty($result2)) ? $result2[0]['last_limit'] : '0';
        ...
        (SELECT @no_of_subcategories := count(`id`) from subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of FROM `category` c ORDER By CAST(c.row_order as unsigned) ASC";
        $db->sql($sql);
        ...
        if (!empty($result)) {
            for ($i = 0; $i < count($result); $i++) {
                ...
                $sql2 = "SELECT * FROM `tbl_last_play_status` WHERE user_id='$user_id'  AND  category_id='" . $result[$i]['id'] . "' AND language_id='0'";
                $db->sql($sql2);
                $result2 = $db->getResult();
                $result[$i]['last_limit'] = (!empty($result2)) ? $result2[0]['last_limit'] : '0';
...
if (isset($_POST['access_key']) && isset($_POST['get_subcategory_by_maincategory'])) {
    ...
    if (isset($_POST['main_id']) && isset($_POST['user_id'])) {
        ...
        $language_id = (isset($_POST['language_id'])) ? $db->escapeString($_POST['language_id']) : 0;
        ...
        if (!empty($result)) {
            for ($i = 0; $i < count($result); $i++) {
                ...
                $sql2 = "SELECT * FROM `tbl_last_play_status` WHERE user_id='$user_id' AND  language_id='$language_id' AND  category_id='$id' AND subcategory_id ='" . $result[$i]['id'] . "'";
                $db->sql($sql2);
                $result2 = $db->getResult();
                $result[$i]['last_limit'] = (!empty($result2)) ? $result2[0]['last_limit'] : '0';
            }
...
if (isset($_POST['access_key']) && isset($_POST['get_questions_by_category'])) {
    ...
    if (isset($_POST['category'])) {
        ...
        $TotalQuestionSql = "SELECT * FROM `question` WHERE category=" . $id . " ORDER BY id ASC";
        $db->sql($TotalQuestionSql);
        $TotalQuestion = $db->getResult();

        $where = '';
        if (isset($_POST['limit']) != '') {
            $limit = $db->escapeString($_POST['limit']);
            $where = "LIMIT " . ($limit) . "," . count($TotalQuestion) . "";
        }

        $sql = "SELECT * FROM `question` WHERE category=" . $id . " ORDER BY id ASC $where";
        $db->sql($sql);
        $result = $db->getResult();
...
if (isset($_POST['access_key']) && isset($_POST['get_questions_by_subcategory'])) {
    ...
    if (isset($_POST['subcategory'])) {
        ...
        $TotalQuestionSql = "SELECT * FROM `question` WHERE subcategory=" . $id . " ORDER BY id ASC";
        $db->sql($TotalQuestionSql);
        $TotalQuestion = $db->getResult();
        ...
        $sql = "SELECT * FROM `question` WHERE subcategory=" . $id . " ORDER BY id ASC $where";
        $db->sql($sql);
        $result = $db->getResult();
    ...
    if (isset($_POST['type']) && isset($_POST['firebase_id']) && ($_POST['firebase_id'] != 'null') && ($_POST['firebase_id'] != 'NULL')) {
        ...
        if ($firebase_id != '') {
            ...
            if ($getuid == TRUE) {
                ...
                $sql = "SELECT * FROM users WHERE firebase_id='$firebase_id' AND email='$email'";
                $db->sql($sql);
                $res = $db->getResult();
                if (!empty($res)) {
...
if (isset($_POST['access_key']) && isset($_POST['get_user_by_id'])) {
    ...
    if (isset($_POST['id'])) {
        ...
        $sql2 = "SELECT spin_count, spin_date FROM `users` WHERE users.id =" . $id . " AND spin_date='" . $toDate . "' ";
        $db->sql($sql2);
        $res2 = $db->getResult();
        if (empty($res2)) {
            $sql4 = "UPDATE `users` SET `spin_count`='0' WHERE `id`='" . $id . "'";
            $db->sql($sql4);
        }
...
if (isset($_POST['access_key']) && isset($_POST['account_remove'])) {
    ...
    if (!empty($_POST['access_key']) && !empty($_POST['user_id'])) {
        ...
        if ($db->sql($sql)) {
            ...
            $sql9 = 'DELETE FROM `tbl_last_play_status` WHERE `user_id`=' . $user_id;
            $db->sql($sql9);
...
// 47. set_last_play_status()
if (isset($_POST['access_key']) && isset($_POST['set_last_play_status'])) {
    if (!verify_token()) {
        return false;
    }
    if ($access_key != $_POST['access_key']) {
    ...
    if (!empty($_POST['access_key']) && !empty($_POST['user_id']) && $_POST['limit'] != '') {
        $user_id = $db->escapeString($_POST['user_id']);
        $category_id = $db->escapeString($_POST['category_id']);
        $subcategory_id = $db->escapeString($_POST['subcategory']);
        $limit = $db->escapeString($_POST['limit']);
        $language_id = ($_POST['language_id'] != '') ? $db->escapeString($_POST['language_id']) : 0;

        $total_question = $db->escapeString($_POST['total_question']);
        $status = (isset($_POST['status'])) ? $db->escapeString($_POST['status']) : 0;

        $sql1 = "SELECT * FROM `tbl_last_play_status` WHERE user_id='$user_id' AND category_id='$category_id' AND subcategory_id='$subcategory_id' AND language_id='$language_id'";
        $db->sql($sql1);
        $result1 = $db->getResult();
        ...
        if (empty($result1)) {
            ...
            $sql = $db->insert('tbl_last_play_status', $data);
            ...
            $where = 'user_id="' . $user_id . '" AND category_id="' . $category_id . '" AND subcategory_id="' . $subcategory_id . '" AND language_id="' . $language_id . '"';
            $sql = $db->update('tbl_last_play_status', $data, $where);
...
// 48. get_last_play_status()
if (isset($_POST['access_key']) && isset($_POST['get_last_play_status'])) {
    if (!verify_token()) {
        return false;
    }
    if ($access_key != $_POST['access_key']) {
    ...
    if (!empty($_POST['access_key']) && !empty($_POST['get_last_play_status'])) {

        $where = $where1 = $join = '';
        if (isset($_POST['user_id']) != '' && isset($_POST['category_id']) != '' && isset($_POST['subcategory']) != '' && isset($_POST['language_id']) != '') {
            ...
            $where = "WHERE tlps.user_id='$user_id' AND tlps.category_id='$category_id' AND tlps.subcategory_id='$subcategory_id' AND tlps.language_id='$language_id'";
        } else if (isset($_POST['user_id']) != '' && isset($_POST['category_id']) != '' && isset($_POST['subcategory']) != '') {

            $user_id = $db->escapeString($_POST['user_id']);
            $category_id = $db->escapeString($_POST['category_id']);
            $subcategory_id = $db->escapeString($_POST['subcategory']);

            $where = "WHERE tlps.user_id='$user_id' AND tlps.category_id='$category_id' AND tlps.subcategory_id='$subcategory_id'";
        } else if (isset($_POST['user_id']) != '' && isset($_POST['category_id']) != '' && isset($_POST['language_id']) != '') {
            ...
            $where = "WHERE tlps.user_id='$user_id' AND tlps.category_id='$category_id' AND  tlps.language_id='$language_id'";
        } else if (isset($_POST['user_id']) != '' && isset($_POST['category_id']) != '') {

            $user_id = $db->escapeString($_POST['user_id']);

            $where = "WHERE tlps.user_id='$user_id' AND tlps.category_id='$category_id'";
        } else if (isset($_POST['user_id']) != '' && isset($_POST['language_id']) != '') {

            $user_id = $db->escapeString($_POST['user_id']);
            $language_id = $db->escapeString($_POST['language_id']);

            $where = "WHERE tlps.user_id='$user_id' AND tlps.status = '0' AND tlps.language_id='$language_id'";
            $join = "LEFT JOIN category c ON c.id=tlps.category_id LEFT JOIN subcategory s ON s.id=tlps.subcategory_id";
            $where1 = ",IFNULL(c.category_name,'') as category_name,IFNULL(s.subcategory_name,'') as subcategory_name,(SELECT COUNT(*) FROM `question` q WHERE q.category=tlps.category_id AND q.subcategory=tlps.subcategory_id) as TotalQuestion";
        } else if (isset($_POST['user_id']) != '') {

            $user_id = $db->escapeString($_POST['user_id']);

            $where = "WHERE tlps.user_id='$user_id' AND tlps.status = '0'";
            $join = "LEFT JOIN category c ON c.id=tlps.category_id LEFT JOIN subcategory s ON s.id=tlps.subcategory_id";
            $where1 = ",IFNULL(c.category_name,'') as category_name,IFNULL(s.subcategory_name,'') as subcategory_name,(SELECT COUNT(*) FROM `question` q WHERE q.category=tlps.category_id AND q.subcategory=tlps.subcategory_id) as TotalQuestion";
        }

        $sql1 = "SELECT tlps.*,IFNULL(c.image,'') as category_image,IFNULL(s.image,'') as subcategory_image $where1 FROM `tbl_last_play_status` tlps LEFT JOIN category c ON c.id=tlps.category_id LEFT JOIN subcategory s ON s.id=tlps.subcategory_id $where";
        $db->sql($sql1);
        $result1 = $db->getResult();
        ...
        if ($result1) {
            ...
            for ($i = 0; $i < count($result1); $i++) {
                $result1[$i]['category_image'] = (!empty($result1[$i]['category_image'])) ? DOMAIN_URL . 'images/category/' . $result1[$i]['category_image'] : '';
                $result1[$i]['subcategory_image'] = (!empty($result1[$i]['subcategory_image'])) ? DOMAIN_URL . 'images/subcategory/' . $result1[$i]['subcategory_image'] : '';

                $sql1 = "SELECT * FROM `tbl_cat_sub_cat_complete` WHERE user_id='$user_id' AND  category='" . $result1[$i]['category_id'] . "' AND subcategory ='" . $result1[$i]['subcategory_id'] . "'";
                $db->sql($sql1);
                $result2 = $db->getResult();
                $result1[$i]['is_played'] = (!empty($result2)) ? '1' : '0';
            }
...
//49. update_spin_count
if (isset($_POST['access_key']) && isset($_POST['update_spin_count'])) {
    if (!verify_token()) {
        return false;
    }
    if ($access_key != $_POST['access_key']) {
    ...
    if (isset($_POST['user_id'])) {
        ...
        $sql1 = "select * from `users` WHERE `id`='" . $id . "' and `spin_date`='" . $toDate . "' ";
        $db->sql($sql1);
        $res1 = $db->getResult();

        $sql2 = "SELECT coin FROM `tbl_custom_coin` WHERE type='daily_max_spin_limit'";
        $db->sql($sql2);
        $res2 = $db->getResult();

        if (!empty($res1)) {
            $old_spin = $res1[0]['spin_count'];
            if ($old_spin < $res2[0]['coin']) {
                $sql = "UPDATE `users` SET `spin_count` = `spin_count` + " . $spin_count . "  WHERE `id` = " . $id;
                $db->sql($sql);
                $response['error'] = "false";
                $response['message'] = "updated successfully";
            } else {
                $response['error'] = "true";
                $response['message'] = "Can not spin more than " . $res2[0]['coin'] . " times";
            }
        } else {
            $sql = "UPDATE `users` SET `spin_count`='" . $spin_count . "' , `spin_date`='" . $toDate . "' WHERE `id`='" . $id . "'";
            $db->sql($sql);
            $response['error'] = "false";
            $response['message'] = "updated successfully";
        }
