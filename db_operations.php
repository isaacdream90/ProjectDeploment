...
/*
  ...
  45. battle_settings
  46. update_payment_request()
  47. update_payment_settings()
  48. custom Coin
  49. upload_firebase_json
...
if (isset($_POST['name']) && isset($_POST['add_category'])) {
    ...
    $category_coin=($_POST['paid_coin']);
    if(!empty($category_coin)){
       $coins=$category_coin;
    } else{
        $coins=0;
    }
    ...
    $sql = "INSERT INTO `category` (`language_id`,`category_name`, `image`,`row_order`,`category_type`,`coins`) VALUES ('" . $language_id . "','" . $name . "','" . $filename . "','0',$category_type,$coins)";
    $db->sql($sql);
    // echo $sql;
...
if (isset($_POST['category_id']) && isset($_POST['update_category'])) {
    ...
    $sql = "Update `category` set `category_name`='" . $name . "',`status`='" . $status . "'";
...
if (isset($_POST['question']) && isset($_POST['add_question'])) {
    ...
    $sql = "INSERT INTO `question`(`category`, `subcategory`, `language_id`, `image`, `question`, `question_type`, `optiona`, `optionb`, `optionc`, `optiond`, `optione`,  `answer`, `note`) VALUES
...
if (isset($_POST['question_id']) && isset($_POST['update_question'])) {
    ...
    $sql = "Update `question` set `question`='" . $question . "', `category`='" . $category . "', `subcategory`='" . $subcategory . "',`question_type`='" . $question_type . "',`optiona`='" . $a . "',`optionb`='" . $b . "' ,`optionc`='" . $c . "' ,`optiond`='" . $d . "', `answer`='" . $answer . "',`note`='" . $note . "'";
...
if (isset($_POST['title']) && isset($_POST['send_notifications'])) {
    ...
    if ($users == 'all') {
        ...
        if (empty($selected_list)) {
            echo '<p class="alert alert-danger">Please Select the users from the table</p>';
            return false;
        }
        $fcm_ids = array();
...
if (isset($_POST['update_fcm_server_key'])) {
    ...
    $fcm_key = trim($db->escapeString($_POST['fcm_key']));
...
if (isset($_POST['import_questions']) && $_POST['import_questions'] == 1) {
    ...
    if ($_FILES["questions_file"]["size"] > 0 && $file_extension == "csv") {
      ...
      // $emapData = fgetcsv($file, 10000, ",");
        // echo count($emapData);//"<pre>".print_r($emapData,TRUE)."</pre>";
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if (count($emapData) > 2) {
                ...
                $emapData[11] = $db->escapeString($emapData[11]);      // note
                $count++;
                if ($count > 1) {
                    if ($emapData[3] == '1') {
                        if ($emapData[0] != '' && $emapData[1] != '' && $emapData[2] != '' && !empty($emapData[3]) && $emapData[4] != '' && $emapData[5] != '' && $emapData[6] != '' && $emapData[7] != '' && $emapData[8] != '' && !empty($emapData[10])) {
                        ...
                        if ($emapData[0] != '' && $emapData[1] != '' && $emapData[2] != '' && !empty($emapData[3]) && $emapData[4] != '' && $emapData[5] != '' && $emapData[6] != '' && !empty($emapData[10])) {
                            $empty_value_found = true;
                        } else {
                            $empty_value_found = false;
                            break;
        ...
        if ($empty_value_found == TRUE) {
            ...
            while (($emapData1 = fgetcsv($file, 10000, ",")) !== FALSE) {
                if (count($emapData1) > 2) {
                    ...
                    if ($count1 > 1) {
                        if (count($emapData1) > 2) {
                            $sql = "INSERT INTO `question`(`category`, `subcategory`, `language_id`, `image`, `question_type`, `question`,`optiona`, `optionb`, `optionc`, `optiond`,  `optione`, `answer`, `note`) VALUES
						('$emapData1[0]','$emapData1[1]','$emapData1[2]','','$emapData1[3]','$emapData1[4]','$emapData1[5]','$emapData1[6]','$emapData1[7]','$emapData1[8]','$emapData1[9]','$emapData1[10]','$emapData1[11]')";
                            $db->sql($sql);
...
if (isset($_POST['update_policy'])) {
    ...
    if (!empty($res)) {
        ...
        $sql = "INSERT INTO `settings`(`type`, `message`, `status`) VALUES ('privacy_policy','" . $message . "',0)";
    }
...
if (isset($_POST['update_terms'])) {
    ...
    $res = $db->getResult();
    if (!empty($res)) {
        $sql = "Update `settings` set `message`='" . $message . "' where `type`='terms_conditions'";
    } else {
        $sql = "INSERT INTO `settings`(`type`, `message`, `status`) VALUES ('terms_conditions','" . $message . "',0)";
    }
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_POST['app_link']) && isset($_POST['system_configurations'])) {
    ...
    foreach ($_POST as $key => $value) {
       if($key != 'system_configurations'){
          $sql = "SELECT * FROM `settings` WHERE type='$key'";
          $db->sql($sql);
          $result = $db->getResult();

          if (empty($result)) {
              $sql = 'INSERT INTO settings (type,message,status) VALUES ("' . $key . '","' . $value . '","0")';
          } else {
              $sql = 'UPDATE settings SET message="' . $value . '" WHERE type="' . $key . '"';
          }
...
if (isset($_POST['update_about_us'])) {
    ...
    if (!empty($res)) {
        ...
        $sql = "INSERT INTO `settings`(`type`, `message`, `status`) VALUES ('about_us','" . $message . "',0)";
    }
...
if (isset($_POST['update_instructions'])) {
    ...
    if (!empty($res)) {
        ...
        $sql = "INSERT INTO `settings`(`type`, `message`, `status`) VALUES ('instructions','" . $message . "',0)";
    }
    $db->sql($sql);
    $res = $db->getResult();
...
// 45. battle_settings()
if (isset($_POST['battle_settings_id']) && isset($_POST['battle_settings'])) {

    $date = $db->escapeString(date('Y-m-d'));
    if (!empty($_POST['battle_settings_id'])) {
        $sql = "UPDATE settings SET message='" . json_encode($_POST, JSON_UNESCAPED_UNICODE) . "' WHERE type='battle_settings'";
    } else {
        $sql = "INSERT INTO settings (type,message,status) VALUES ('battle_settings','" . json_encode($_POST, JSON_UNESCAPED_UNICODE) . "','1')";
    }
    $db->sql($sql);
    $res = $db->getResult();
    echo "<p class='alert alert-success'>Settings Saved!</p>";
...
//46. update_payment_request()
if (isset($_POST['uid']) && isset($_POST['update_payment_request'])) {
    ...
    $sql = "select `status` from `payment_requests` where `uid`='$uid' and `id` = $id";
    $db->sql($sql);
    $res = $db->getResult();

    if ($res[0]['status'] == 2) {
        echo "<p class='alert alert-warning'>Oops! Can't update status. Once its done.</p>";
        return false;
    }

    $sql = "select * from `users` where `firebase_id`='$uid'";
    $db->sql($sql);
    $UserResult = $db->getResult();

    if ($status == 2) {

        $sql = "update users set `coins` = `coins` + $points_used where `firebase_id`='$uid'";
        $db->sql($sql);

        $data = array(
            'uid' => $uid,
            'points' => $points_used,
            'type' => 'Reversed by admin',
            'type_two' => 'Reversed by admin',
            'coin_status' => '0',
    ...
    $sql = "Update `payment_requests` set `remarks`='" . $remarks . "', `status`='" . $status . "' where `uid`='" . $uid . "' and `id`=" . $id;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    array_push($fcm_ids, $UserResult[0]['fcm_id']);

    $maxlevel = $no_of = "0";
    $maincat_id = 0;
    if ($status == 1) {
        $title = 'Payment Request Complete';
        $message = "Your Payment is Complete..You have used " . $points_used . " Points. Thank You!!!";
        $type = 'default';
        ...
        $message = "Your Payment Details is Wrong..we have Refund Your " . $points_used . " Points. Thank You!!!";
        $type = 'default';
        $maxlevel = $no_of = "0";
...
if (isset($_POST['update_bulk_payment_request'])) {
    $ids = $_POST['ids'];
    $status = $db->escapeString($_POST['status']);
    if ($ids == '') {
        echo "<p class='alert alert-warning'>Please select some records before submiting!</p>";
        return false;
    }

    $sql = "UPDATE `payment_requests` SET `status`=$status where `id` in ($ids) and (`status` = 0 or `status` = 1)";
    $db->sql($sql);
    $res = $db->getResult();
    echo "<p class='alert alert-success'>Payment Status updated for all!</p>";
}
...
if (isset($_POST['update_payment_settings'])) {
    ...
    if (empty($result)) {
        $sql = "INSERT INTO settings (type,message,status) VALUES ('pay_setting','" . $message . "','" . $status . "')";
    } else {
        $sql = "Update `settings` set  `message`='" . $message . "', `status`='" . $status . "' where `type`='pay_setting'";
    }

    $db->sql($sql);
    $res = $db->getResult();
    echo "<p class='alert alert-success'>Payment Settings Updated Successfully!</p><br>";
}


//48. custom coin()
if (isset($_POST['custom_coin'])) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    ...
    if (!empty($_POST)) {
        foreach ($_POST as $key => $value) {
            ...
            if($key != 'custom_coin'){
                $sql = "SELECT * FROM `tbl_custom_coin` WHERE type='$key'";
                $db->sql($sql);
                $result = $db->getResult();
                ...
                if (empty($result)) {
                    $sql = "INSERT INTO tbl_custom_coin (type,coin) VALUES ('" . $key . "','" . $value . "')";
                } else {
                    $sql = "UPDATE tbl_custom_coin SET coin='" . $value . "' WHERE type='" . $key . "'";
                }
    ...
    echo "<p class='alert alert-success'> Coin updated Successfully!</p><br>";
}
...
if (isset($_POST['update_system'])) {
    ...
    if (isset($_POST['purchase_code']) && isset($_POST['quiz_url'])) {
        if (!empty($_POST['purchase_code']) && !empty($_POST['quiz_url'])) {
            ...
            //$quiz_url = $db->escapeString($_POST['quiz_url']);
            $quiz_url = $_SERVER['HTTP_HOST'].str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://wrteam.in/validator/qearner_quiz_validator?purchase_code=' . $purchase_code . '&domain_url=' . $quiz_url,
...
//49. upload_firebase_json()
if (isset($_POST['upload_firebase_json'])) {
    if($_FILES['file']['name'] != ''){
        $target_path = getcwd() . DIRECTORY_SEPARATOR;
        $target_path1 = $target_path . '/firebase';
        $extension  = pathinfo( $_FILES["file"]["name"], PATHINFO_EXTENSION ); // json
        $newfileName   =  'firebase_credentials' . "." . $extension; // 5dab1961e93a7_1571494241.jpg

        $upload_file = $target_path1 .'/'.$newfileName;
        if($extension == 'json'){
            if(file_exists($upload_file)){
                unlink($upload_file);
            }
            move_uploaded_file($_FILES["file"]["tmp_name"],$upload_file);
            //chmod($target_path1 .'/'.$newfileName, 0775);
            $result = '<label class="alert alert-success">Upload successfully!</label>';
        }else{
            $result = "<label class='alert alert-danger'>Invalid file, please try again.!<lable>";
        }
    }else{
        $result = "<label class='alert alert-danger'>File Not Uploaded<lable>";
    }
    echo $result;
}


if (isset($_POST['user_id_coin']) && isset($_POST['update_user1'])) {
    ...
    $id = $_POST['user_id_coin'];
    $new_coins = $db->escapeString($_POST['coins']);

    $user_data="SELECT firebase_id FROM users where id=".$id;
    $db->sql($user_data);
    $user_res= $db->getResult();
  
    $user_insert = "INSERT INTO `tbl_tracker` (`user_id`, `uid`, `points`, `type`, `type_two`, `coin_status`, `date`) VALUES ('" . $id . "','" . $user_res[0]["firebase_id"] . "','" . $new_coins . "','Admin Added','Admin added','0','". date("Y-m-d")."')";
    $db->sql($user_insert);
    ...
    $sql="SELECT coins FROM users where id=".$id;
    $db->sql($sql);
    $res = $db->getResult();
   
    $old_coins=$res[0]['coins'];


    $total_coins=$old_coins+$new_coins;
    
    $sql2 = "Update users set `coins`='" . $total_coins . "' where `id`=" . $id;
    $db->sql($sql2);
    $res2 = $db->getResult();
    
    echo "<p class='alert alert-success'>User Coins Added!</p>";
}
